...
\textbf{Acceptance criteria.}
\begin{itemize}
  \item Given the defense evidence package is prepared, when the demonstration starts, then representative data is already available.
  \item Given live model behavior is unstable, when embedded screenshots and exported artifacts are used, then the key flows can still be reviewed by the examiner.
...
\textbf{Edge and error cases.}
\begin{itemize}
  \item Given the backend is unavailable during a live demonstration, when the screenshot and API evidence package is available, then the presenter can still explain completed functionality.
  ...
  \item Given an image is unavailable during compilation, when the LaTeX source is built, then the figure fallback identifies the expected asset instead of failing the document build.
\end{itemize}
