...
The recommended final walkthrough order is:
...
\begin{enumerate}
  \item Export PlantUML diagrams to PNG files.
  \item Capture all screenshots listed in Appendix A.
  ...
  \item Verify that every placeholder has either been replaced by an image or intentionally kept as a draft marker.
  \item Send the compiled PDF and source folder to the supervisor for final review.
\end{enumerate}

\section{If the Page Count Is Too Low}

If the compiled document is below the target range, the best expansion areas are:

\begin{itemize}
  \item add quantitative latency and cost tables to Chapter 6;
  \item add a small usability questionnaire summary if available;
  \item expand Chapter 5 with endpoint-by-endpoint implementation details;
  \item include more final screenshots;
  \item keep Appendix C and Appendix D in the submitted PDF.
\end{itemize}

\section{If the Page Count Is Too High}

If the compiled document exceeds the target range, reduce length in this order:

\begin{enumerate}
  \item move Appendix D to supporting material outside the PDF;
  \item shorten Appendix C by keeping only story summaries;
  \item reduce repeated implementation explanation in Chapter 5;
  \item keep diagrams and screenshots because they are higher-value evidence than repeated prose.
\end{enumerate}

\section{Operational Summary}

The operational review process should demonstrate that ClearMind is not only a written design. The system has runnable frontend and backend components, persistent data models, AI-assisted workflows, API documentation, tests, coverage evidence, and a prepared final thesis narrative. The screenshots and recorded walkthrough provide resilience for defense conditions where live services may be unreliable.
