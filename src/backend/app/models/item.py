...
class Item(Base):
    ...
    links_from = relationship(
        "ItemLink", foreign_keys="ItemLink.source_id", back_populates="source", cascade="all, delete-orphan"
    )
    links_to = relationship(
        "ItemLink", foreign_keys="ItemLink.target_id", back_populates="target", cascade="all, delete-orphan"
    )
